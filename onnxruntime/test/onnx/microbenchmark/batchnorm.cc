...
static void BM_BatchNorm(benchmark::State& state) {
  concurrency::ThreadPool tp(&onnxruntime::Env::Default(), concurrency::ThreadOptions(), "", 4, false);
...
BENCHMARK(BM_BatchNorm)->Arg(1)->Arg(16)->Arg(64)->UseRealTime()->Unit(benchmark::TimeUnit::kMicrosecond);

static void BM_BatchNormSingleBatch(benchmark::State& state) {
  concurrency::ThreadPool tp(&onnxruntime::Env::Default(), concurrency::ThreadOptions(), "", 4, false);
...
BENCHMARK(BM_BatchNormSingleBatch)->UseRealTime()->Unit(benchmark::TimeUnit::kMicrosecond);